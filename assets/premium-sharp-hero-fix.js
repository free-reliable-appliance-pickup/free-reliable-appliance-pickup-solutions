...
function applyFallback(){const p=pagePath();if(!/-washer-dryer-pickup\/$/.test(p))return;const img=hero();if(!img||img.dataset.userProvidedHero==='1')return;let hash=0;for(let i=0;i<p.length;i++)hash=(hash*31+p.charCodeAt(i))>>>0;const file=fallbackPhotos[hash%fallbackPhotos.length];img.src=base()+'assets/laundry/'+file+'?full-res-fallback=20260917';const city=cityLabel();if(city)img.alt='Real washer and dryer set for pickup in '+city;img.dataset.fullResolutionFallback='1';styleHero(img);}
async function applyExactCityHero(){const p=pagePath();if(!/-washer-dryer-pickup\/$/.test(p))return;const img=hero();if(!img)return;try{const res=await fetch(base()+'assets/premium-user-heroes.json?exact-user-heroes=20260917',{cache:'no-store'});if(!res.ok)return;const map=await res.json();const cfg=map[p];if(!cfg)return;if(cfg.src){img.src=base()+cfg.src+'?exact-user-hero=20260917';}else if(Array.isArray(cfg.parts)&&cfg.parts.length){const chunks=await Promise.all(cfg.parts.map(part=>fetch(base()+part+'?exact-user-hero=20260917',{cache:'force-cache'}).then(r=>{if(!r.ok)throw new Error('hero part');return r.text();})));img.src='data:'+(cfg.mime||'image/jpeg')+';base64,'+chunks.map(s=>s.trim()).join('');}else{return;}if(cfg.alt)img.alt=cfg.alt;img.dataset.userProvidedHero='1';img.dataset.fullResolutionHero='1';delete img.dataset.fullResolutionFallback;styleHero(img);}catch(e){}}
